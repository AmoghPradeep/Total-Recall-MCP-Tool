...
`python -m pip install -e .[test]` installs the package plus pytest. `pytest` runs the full suite. Use the unit or integration path filters for faster iteration. `obsidian-rag-background` starts the vault watcher and ingestion worker; `obsidian-rag-mcp-server` starts the JSON-RPC MCP server over stdio.

## Coding Style & Naming Conventions
Follow the existing Python style: 4-space indentation, type hints on public functions, and small focused modules. Use `snake_case` for files, functions, and variables; `PascalCase` for Pydantic models and service classes; `UPPER_CASE` for constants such as `DEFAULT_ENV_FILE`. Prefer explicit imports from `obsidian_rag_mcp.*` and keep platform-aware config/path logic centralized in `config.py` or dedicated utilities.
...
Configuration loads from `OBRAG_` environment variables, then `~/.obragconfig/.env`, then code defaults. Do not commit local vault paths, model endpoints, or generated SQLite/manifest data. If you change startup or operational behavior, update `docs/runbook.md` in the same PR.
